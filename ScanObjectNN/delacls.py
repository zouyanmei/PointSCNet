...
class LFP(nn.Module):
    ...
    def forward(self, x, knn, nbr=None):
        B, N, C = x.shape
        x = self.proj(x)
        '''
        if nbr is not None:
...
class Block(nn.Module):
    ...
    def forward(self, x, knn, nbr=None):
        ...
        for i in range(self.depth):
            x = x + self.drop_paths[i](self.lfps[i](x, knn, nbr))
            if i % 2 == 1:
...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        self.x_embed = nn.Sequential(
            nn.Linear(x_dim, nbr_out_dim, bias=False),
            nn.BatchNorm1d(nbr_out_dim, momentum=args.bn_momentum),
            args.act(),
            nn.Linear(nbr_out_dim, nbr_out_dim, bias=False),
            #nn.BatchNorm1d(nbr_hid_dim, momentum=args.bn_momentum),
            #args.act(),
            #nn.Linear(nbr_hid_dim, nbr_out_dim, bias=False)
        )
        ...
        self.dcd = DCDLayer(dim, args=args, ration=4)
    
    def forward(self, x, xyz, prev_knn, pwd, nbr=None):
        """
        x: B x N x C
        """
        # downsampling
        ...
        x = self.dcd(nbr) if self.first else self.dcd(nbr) + x
        #if self.first:
        #    x = self.dcd(x)
        # main block
        x = self.blk(x, knn, nbr)
...
class DCDLayer(nn.Module):
    ...
    x:n, c
    '''
    def __init__(self, in_channels, args, ration=4):
    ...
    def forward(self, x):
        ...
        x2: b, n, c
        '''            
        mean_f = torch.max(x, 1, keepdim=False)[0] #b*c

        #out_mean = self.fc(mean_f).unsqueeze(1)
        out_w = self.sigmoid(self.fc1(mean_f)).unsqueeze(1)
