...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        if not last:
            ...
            self.dcd = DCDLayer(args=args, ration=8)
...
class DCDLayer(nn.Module):
    ...
    def forward(self, x, npoint):
        '''
        x1:n, c
        x2:n, c, best
        '''
        batch_size = len(npoint)
        cur_feature = x[0:npoint[0], :]
        mean_f = torch.mean(cur_feature, 0, keepdim=True)
        for i in range(1, batch_size):
            cur_feature = x[sum(npoint[0:i]):sum(npoint[0:(i+1)]), :]
