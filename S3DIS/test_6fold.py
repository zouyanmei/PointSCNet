import torch
from torch import nn
import torch.nn.functional as F
from s3dis import S3DIS, s3dis_test_collate_fn
from torch.utils.data import DataLoader
import sys
import os
from pathlib import Path
sys.path.append(str(Path(__file__).absolute().parent.parent))
import utils.util as util
from delasemseg import DelaSemSeg
from config import s3dis_args, dela_args
from torch.cuda.amp import autocast
import numpy as np
import random

torch.set_float32_matmul_precision("high")
def seed_torch(seed=0):
	random.seed(seed)
	os.environ['PYTHONHASHSEED'] = str(seed) 
	np.random.seed(seed)
	torch.manual_seed(seed)
	torch.cuda.manual_seed(seed)
	torch.cuda.manual_seed_all(seed)
	torch.backends.cudnn.benchmark = False
	torch.backends.cudnn.deterministic = True
    #torch.use_deterministic_algorithms(True) 

seed_torch()

#loop = 12
loop = 1
os.makedirs(f"6flod/test", exist_ok=True)
logfile = f"6flod/test/test_novoting.log"
logfile = open(logfile, "a", 1)
sys.stdout = logfile
metric_all = util.Metric(13)
for i in range(1,7):
    partition = f"{i}"
    print(f'================ Area {i} ================')
    testdlr = DataLoader(S3DIS(s3dis_args, partition=partition, loop=loop, train=False, test=True), batch_size=1,
                      collate_fn=s3dis_test_collate_fn, pin_memory=True, num_workers=8)
    model = DelaSemSeg(dela_args).cuda()
    util.load_state(f"6flod/Area{i}/best.pt", model=model)
    model.eval()
    metric = util.Metric(13)
    cum = 0
    cnt = 0
    with torch.no_grad():
        for xyz, feature, indices, nn, y, npoint in testdlr:
            xyz = xyz.cuda(non_blocking=True)
            feature = feature.cuda(non_blocking=True)
            indices = [ii.cuda(non_blocking=True).long() for ii in indices[::-1]]
            nn = nn.cuda(non_blocking=True).long()
            with autocast():
                p = model(xyz, feature, indices, None, [npoint])
            cum = cum + p[nn]
            cnt += 1
            if cnt % loop == 0:
                y = y.cuda(non_blocking=True)
                metric.update(cum, y)
                metric_all.update(cum, y)
                cnt = cum = 0
    metric.print("test: ")
    metric.reset()
    print(f'================ End of Area {i} ================\n\n')
print(f'================ 6-FLOD ================')
metric_all.print("test: ")
metric_all.reset()
print(f'================ End of 6-FLOD ================\n\n')