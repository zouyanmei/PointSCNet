...
def warmup_fn(model, dataset):
    ...
    traindlr = DataLoader(dataset, batch_size, collate_fn=s3dis_collate_fn, pin_memory=True, num_workers=6)
    for xyz, feature, indices, pts, y, npoint in traindlr:
        ...
        with autocast():
            p, closs = model(xyz, feature, indices, pts, npoint)
...
traindlr_raw = S3DIS(s3dis_args, partition="!5", loop=30)
testdlr_raw = S3DIS(s3dis_args, partition="5", loop=1, train=False)

traindlr = DataLoader(traindlr_raw, batch_size=batch_size, 
                      collate_fn=s3dis_collate_fn, shuffle=True, pin_memory=True, 
                      persistent_workers=True, drop_last=True, num_workers=16)
testdlr = DataLoader(testdlr_raw, batch_size=1,
...
for i in range(start_epoch, epoch):
    ...
    with torch.no_grad():
        z = 0
        for xyz, feature, indices, pts, y, npoint in testdlr:
            xyz = xyz.cuda(non_blocking=True)
            feature = feature.cuda(non_blocking=True)
            indices = [ii.cuda(non_blocking=True).long() for ii in indices[::-1]]
            y = y.cuda(non_blocking=True)
            with autocast():
                p = model(xyz, feature, indices, None, npoint)
            metric.update(p, y)
            z = z+1
