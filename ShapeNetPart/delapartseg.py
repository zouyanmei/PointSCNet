...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        self.x_embed = nn.Sequential(
            ...
            nn.BatchNorm1d(nbr_hid_dim//2, momentum=args.bn_momentum),
            args.act(),
            nn.Linear(nbr_hid_dim//2, nbr_out_dim, bias=False),
            #nn.BatchNorm1d(nbr_hid_dim, momentum=args.bn_momentum),
            #args.act(),
            #nn.Linear(nbr_hid_dim, nbr_out_dim, bias=False),
        )
        #self.dcd = DCDLayer(args=args, in_channels=dim, ration=2)
        ...
        if not last:
            ...
            self.dcd = DCDLayer(args=args, in_channels=args.head_dim, ration=4)
    
    def forward(self, x, xyz, prev_knn, pwd,shape):
        """
        x: B x N x C
        """
        # downsampling
        ...
        if self.first:
            height = xyz[..., 1:2] / 10
            x_em = torch.cat([xyz, height], dim=-1)
        ...
        x_em = self.x_embed(x_em.view(B*N, -1))
        x_em = get_vertex_feature(x_em.view(B, N, -1), knn)
        #nbr = self.nbr_embed(nbr).view(B*N, k, -1).max(dim=1)[0]
        nbr = self.nbr_embed(nbr).view(B*N, k, -1)
        nbr = (nbr+x_em).max(dim=1)[0]
        ...
        # next stage
        if not self.last:
            sub_x, sub_c = self.sub_stage(x, xyz, knn, pwd,shape)
        else:
            sub_x = None
            sub_c = None
...
class DCDLayer(nn.Module):
    ...
    x:n, c
    '''
    def __init__(self, args, in_channels, ration=4):
    ...
    def forward(self, x, shape):
        ...
        '''           
        #print(shape.shape) 
        mean_f = torch.mean(shape, 1, keepdim=False) #b*c
        #max_f = torch.max(x, 1, keepdim=False)[0] #b*c

        out_mean = self.fc(mean_f).unsqueeze(1)
        out_w = self.fc1(mean_f).unsqueeze(1)
        
        x = out_w*x*0.5 + x*0.75
        #+out_mean
        #x = x+out_mean
        return x
...
class DelaPartSeg(nn.Module):
    ...
    def forward(self, xyz, normal, shape):
        ...
        shape = F.one_hot(shape, self.shape_classes).float()
        shape = self.shapeproj(shape).view(B, 1, -1).repeat(1, N, 1).view(B*N, -1)
        x, closs = self.stage(normal, xyz, None, pwd, shape)
        x = x+shape
        x = self.head(x)
        if self.training:
            return x, ridx, closs
